...
class NameVerifyController: SignUpBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        addMainConstraint(tf: ScreenHeigh/2 - numBoardSmall/2 - inset, nxt: 45)
        textField1?.placeholder = ph3
        textField2 = UITextField()
        textField2?.clearButtonMode = .whileEditing
        textField2?.textAlignment = .center
        textField2?.font = UIFont.systemFont(ofSize: 21)
        ...
        labelTitle2.textColor = UIColor.lightGray.withAlphaComponent(0)
    ...
    @objc override func valueChanged() {
        ...
        labelTitle?.textColor = txtLen == 0 ?
            UIColor.clear : UIColor.lightGray
    ...
    ///下移动画
    func drag(duration: Double, _ color: UIColor = UIColor.lightGray) {
    ...
    func unDrag(duration: Double) {
        ...
        UIView.animate(withDuration: duration, animations: {
            self.textField2?.snp.updateConstraints{ make in
                ...
                self.labelTitle2.textColor = UIColor.lightGray.withAlphaComponent(0)
            }
            self.view.layoutIfNeeded()
        })
    }
